...
int Evaluate(string baseDir,string listFile,string detFile,string annotFile);
int main(int argc, char *argv[]){
	...
	/*for(int i=1;i<=10;i++)
	{
		sprintf(list,listFile,i);
		sprintf(det,detFile,i);
		sprintf(annot,annotFile,i);
		
		Evaluate(baseDir,list,det,annot);
	}*/

	Evaluate(baseDir,listFile,detFile,annotFile);
	cout<< faces1 <<"		"<<faces2<<endl;
	return 0;
}
